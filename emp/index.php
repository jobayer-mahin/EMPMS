...
if(isset($_POST['submit'])) {
  ...
  if($email != "" && $password != "") {
    try {
      $query = "SELECT id, EmpId, fname, lname, department_name, email, mobile, country, state, city, address, photo, dob, date_of_joining, password 
                FROM tblemployee 
...
if(isset($_POST['reset'])) {
  ...
  $sql ="SELECT EmpId FROM tblemployee WHERE EmpId=:empid AND mobile=:mobile";
  ...
  if($query->rowCount() > 0) {
    $con = "UPDATE tblemployee SET password=:newpassword WHERE EmpId=:empid AND mobile=:mobile";
    ...
    echo "<script>alert('Employee ID or Mobile number is invalid');</script>"; 
  }
}
?>
    ...
    <section class="login-content" style="height:500px;">
      ...
      <div class="login-box">
        ...
        <form class="login-form" method="post">
          <h3 class="login-head"><i class="fa fa-lg fa-fw fa-user"></i> Sign In | Employee</h3>

          <!-- Login Message -->
          <?php if($msg){ ?>
            <div class="alert alert-danger text-center" role="alert">
              <i class="fa fa-exclamation-circle"></i> <?php echo htmlentities($msg); ?>
            </div>
          <?php } ?>
        ...
        <form class="forget-form" method="post">
          ...
          <div class="form-group">
            ...
            <input class="form-control" type="text" placeholder="Emp Id" name="empid" required>
          </div>
          ...
          <div class="form-group mt-3">
            <p class="semibold-text mb-0">
              <a href="#" data-toggle="flip"><i class="fa fa-angle-left fa-fw"></i> Back to Login</a>
            </p>
          </div>
        </form>
      </div>
